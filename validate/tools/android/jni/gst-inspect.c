...
n_print (const char *format, ...)
{
  ...
  if (_name)
    g_string_append_printf (to_print, "%s", _name);

  va_start (args, format);
  g_string_append_vprintf (to_print, format, args);
  va_end (args);
}
...
print_hierarchy (GType type, gint level, gint * maxlevel)
{
  ...
  *maxlevel = *maxlevel + 1;
  ...
  for (i = 1; i < *maxlevel - level; i++)
    g_string_append_printf (to_print, "      ");
  if (*maxlevel - level)
    g_string_append_printf (to_print, " +----");

  g_string_append_printf (to_print, "%s\n", g_type_name (type));
...
print_interfaces (GType type)
{
  ...
  if (ifaces) {
    if (n_ifaces) {
      if (_name)
        g_string_append_printf (to_print, "%s", _name);
      g_string_append_printf (to_print, "Implemented Interfaces:\n");
      iface = ifaces;
      while (*iface) {
        if (_name)
          g_string_append_printf (to_print, "%s", _name);
...
print_element_properties_info (GstElement * element)
{
  ...
  for (i = 0; i < num_properties; i++) {
    ...
    if (param->flags & G_PARAM_READABLE) {
      ...
      readable = TRUE;
      g_string_append_printf (to_print, "%s%s", (first_flag) ? "" : ", ",
          "readable");
      first_flag = FALSE;
    } else {
    ...
    if (param->flags & GST_PARAM_CONTROLLABLE) {
      g_string_append_printf (to_print, ", %s", "controllable");
      first_flag = FALSE;
    }
    ...
    if (param->flags & ~KNOWN_PARAM_FLAGS) {
      g_string_append_printf (to_print, "%s0x%0x", (first_flag) ? "" : ", ",
    ...
    switch (G_VALUE_TYPE (&value)) {
      case G_TYPE_STRING:
      {
        ...
        if (string_val == NULL)
          g_string_append_printf (to_print, "Default: null");
        else
          g_string_append_printf (to_print, "Default: \"%s\"", string_val);
        break;
      }
      case G_TYPE_BOOLEAN:
      {
        ...
        g_string_append_printf (to_print, "Default: %s",
            bool_val ? "true" : "false");
        break;
      }
      case G_TYPE_ULONG:
      {
        ...
        g_string_append_printf (to_print, "Range: %lu - %lu Default: %lu ",
            pulong->minimum, pulong->maximum, g_value_get_ulong (&value));

        g_printerr ("%s: property '%s' of type ulong: consider changing to "
      ...
      case G_TYPE_LONG:
      {
        ...
        n_print ("%-23.23s Long. ", "");
        g_string_append_printf (to_print, "Range: %ld - %ld Default: %ld ",
            plong->minimum, plong->maximum, g_value_get_long (&value));

        g_printerr ("%s: property '%s' of type long: consider changing to "
      ...
      case G_TYPE_UINT:
      {
        ...
        g_string_append_printf (to_print, "Range: %u - %u Default: %u ",
      ...
      case G_TYPE_INT:
      {
        ...
        g_string_append_printf (to_print, "Range: %d - %d Default: %d ",
      ...
      case G_TYPE_UINT64:
      {
        ...
        g_string_append_printf (to_print,
            "Range: %" G_GUINT64_FORMAT " - %" G_GUINT64_FORMAT " Default: %"
            G_GUINT64_FORMAT " ", puint64->minimum, puint64->maximum,
      ...
      case G_TYPE_INT64:
      {
        ...
        g_string_append_printf (to_print,
            "Range: %" G_GINT64_FORMAT " - %" G_GINT64_FORMAT " Default: %"
            G_GINT64_FORMAT " ", pint64->minimum, pint64->maximum,
      ...
      case G_TYPE_FLOAT:
      {
        ...
        g_string_append_printf (to_print,
            "Range: %15.7g - %15.7g Default: %15.7g ", pfloat->minimum,
            pfloat->maximum, g_value_get_float (&value));
        break;
      }
      case G_TYPE_DOUBLE:
      {
        ...
        g_string_append_printf (to_print,
            "Range: %15.7g - %15.7g Default: %15.7g ", pdouble->minimum,
            pdouble->maximum, g_value_get_double (&value));
        break;
      }
      case G_TYPE_CHAR:
      case G_TYPE_UCHAR:
        g_printerr ("%s: property '%s' of type char: consider changing to "
      ...
      default:
        if (param->value_type == GST_TYPE_CAPS) {
          ...
          while (values[j].value_name) {
            ...
            if (_name)
              g_string_append_printf (to_print, "%s", _name);
            g_string_append_printf (to_print, "%-23.23s    (%d): %-16s - %s",
          ...
          while (vals[0].value_name) {
            ...
            if (_name)
              g_string_append_printf (to_print, "%s", _name);
            g_string_append_printf (to_print,
                "%-23.23s    (0x%08x): %-16s - %s", "", vals[0].value,
                vals[0].value_nick, vals[0].value_name);
            ++vals;
          }
          ...
          g_string_append_printf (to_print,
              "Range: %d/%d - %d/%d Default: %d/%d ", pfraction->min_num,
              pfraction->min_den, pfraction->max_num, pfraction->max_den,
    ...
    if (!readable)
      g_string_append_printf (to_print, " Write only\n");
    else
      g_string_append_printf (to_print, "\n");
...
print_pad_info (GstElement * element)
{
  ...
  while (pads) {
    ...
    if (gst_pad_get_direction (pad) == GST_PAD_SRC)
      g_string_append_printf (to_print, "  SRC: '%s'", name);
    else if (gst_pad_get_direction (pad) == GST_PAD_SINK)
      g_string_append_printf (to_print, "  SINK: '%s'", name);
    else
      g_string_append_printf (to_print, "  UNKNOWN!!!: '%s'", name);
...
print_blacklist (void)
{
  ...
  g_string_append_printf (to_print, "\n");
  g_string_append_printf (to_print, "Total count: ");
  g_string_append_printf (to_print, "%d blacklisted files", count);
  g_string_append_printf (to_print, "\n");
...
print_element_list (gboolean print_all)
{
    ...
    while (features) {
      ...
      if (GST_IS_ELEMENT_FACTORY (feature)) {
        ...
        if (print_all)
          ...
          g_string_append_printf (to_print, "%s:  %s: %s\n",
        ...
        if (!print_all)
          g_string_append_printf (to_print, "%s: %s: ",
              gst_plugin_get_name (plugin),
        ...
        if (extensions != NULL) {
          ...
          while (extensions[i]) {
            if (!print_all)
              g_string_append_printf (to_print, "%s%s", i > 0 ? ", " : "",
                  extensions[i]);
            i++;
          }
          if (!print_all)
          ...
          if (!print_all)
            g_string_append_printf (to_print, "no extensions\n");
        }
      } else {
        if (!print_all)
          n_print ("%s:  %s (%s)\n", gst_plugin_get_name (plugin),
  ...
  g_string_append_printf (to_print, "\n");
  g_string_append_printf (to_print, "Total count: ");
  g_string_append_printf (to_print, "%d plugins", plugincount);
  if (blacklistcount) {
    g_string_append_printf (to_print, " (");
    g_string_append_printf (to_print, "%d blacklist entry", blacklistcount);
    g_string_append_printf (to_print, " not shown)");
  }
  g_string_append_printf (to_print, ", ");
  g_string_append_printf (to_print, "%d feature", featurecount);
  g_string_append_printf (to_print, "\n");
}
  ...
  for (p = plugins; p; p = p->next) {
    ...
    for (f = features; f; f = f->next) {
      ...
      if (GST_IS_ELEMENT_FACTORY (feature)) {
        ...
        if (!factory) {
          g_string_append_printf (to_print,
              "element plugin %s couldn't be loaded\n",
              gst_plugin_get_name (plugin));
          continue;
        }
        ...
        if (!element) {
          g_string_append_printf (to_print,
              "couldn't construct element for %s for some reason\n",
        ...
        if (GST_IS_URI_HANDLER (element)) {
          ...
          g_string_append_printf (to_print, "%s (%s, rank %u): %s\n",
  ...
  while (features) {
    ...
    if (GST_IS_ELEMENT_FACTORY (feature)) {
      ...
      if (extensions) {
        ...
        g_string_append_printf (to_print, "  %s: %s: no extensions\n",
            gst_plugin_get_name (plugin),
            gst_plugin_feature_get_name (feature));
...
print_element_info (GstElementFactory * factory, gboolean print_names)
{
  ...
  if (!factory) {
    g_string_append_printf (to_print, "element plugin couldn't be loaded\n");
    return -1;
  }
    ...
    switch (gst_element_factory_get_uri_type (factory)) {
      case GST_URI_SINK:
        while (*protocols != NULL) {
          g_string_append_printf (to_print, "urisink-%s\n", *protocols);
          ++protocols;
        }
        break;
      case GST_URI_SRC:
        while (*protocols != NULL) {
          g_string_append_printf (to_print, "urisource-%s\n", *protocols);
          ++protocols;
        }
        break;
      default:
...
print_plugin_automatic_install_info (GstPlugin * plugin)
{
  ...
  for (l = features; l != NULL; l = l->next) {
    ...
    if (feature_plugin == plugin) {
      ...
      g_string_append_printf (to_print, "element-%s\n",
          gst_plugin_feature_get_name (feature));
...
_print_all (void)
{
  ...
  g_string_free (to_print, TRUE);
}

int
gst_inspect (int argc, char *argv[])
{
