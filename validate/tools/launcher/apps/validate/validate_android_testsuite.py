...
def register_default_blacklist(self):
    self.set_default_blacklist([
        # hls known issues
        ("validateandroid.hls.playback.fast_forward.*",
         "https://bugzilla.gnome.org/show_bug.cgi?id=698155"),
        ("validateandroid.hls.playback.seek_with_stop.*",
         "https://bugzilla.gnome.org/show_bug.cgi?id=723268"),
        ("validateandroid.hls.playback.reverse_playback.*",
         "https://bugzilla.gnome.org/show_bug.cgi?id=702595"),
        ("validateandroid.hls.*scrub_forward_seeking.*",
         "https://bugzilla.gnome.org/show_bug.cgi?id=606382"),
        ("validateandroid.hls.*seek_backward.*",
         "https://bugzilla.gnome.org/show_bug.cgi?id=606382"),
        ("validateandroid.hls.*seek_forward.*",
         "https://bugzilla.gnome.org/show_bug.cgi?id=606382"),

        # Matroska/WEBM known issues:
        ("validateandroid.*.reverse_playback.*webm$",
         "https://bugzilla.gnome.org/show_bug.cgi?id=679250"),
        ("validateandroid.*.reverse_playback.*mkv$",
         "https://bugzilla.gnome.org/show_bug.cgi?id=679250"),
        ("validateandroid.*reverse.*Sintel_2010_720p_mkv",
        ...
        ("validateandroid.http.playback.seek_with_stop.*webm",
         "matroskademux.gst_matroska_demux_handle_seek_push: Seek end-time not supported in streaming mode"),
        ("validateandroid.http.playback.seek_with_stop.*mkv",
         "matroskademux.gst_matroska_demux_handle_seek_push: Seek end-time not supported in streaming mode"),

        # MPEG TS known issues:
        ('(?i)validateandroid.*(?:_|.)(?:|m)ts$',
         "https://bugzilla.gnome.org/show_bug.cgi?id=702595"),
        ('validateandroid.file.transcode.to_vorbis_and_vp8_in_webm.GH1_00094_1920x1280_MTS',
        ...
        # HTTP known issues:
        ("validateandroid.http.*scrub_forward_seeking.*", "This is not stable enough for now."),
        ("validateandroid.http.playback.change_state_intensive.raw_video_mov",
        ...
        ("validateandroid\.file\.transcode.*mxf", "FIXME: Transcoding and mixing tests need to be tested"),

        # Subtitles known issues
        ("validateandroid.file.playback.switch_subtitle_track.Sintel_2010_720p_mkv", "https://bugzilla.gnome.org/show_bug.cgi?id=734051"),

        # Videomixing known issues
        ("validateandroid.file.*.simple.scrub_forward_seeking.synchronized", "https://bugzilla.gnome.org/show_bug.cgi?id=734060"),
...
try:
    GstValidateAndroidTestManager.register_defaults = register_defaults
    GstValidateAndroidTestManager.register_all = register_all
    GstValidateAndroidTestManager.register_default_blacklist = register_default_blacklist
    GstValidateAndroidTestManager.register_default_test_generators = register_default_test_generators
    GstValidateAndroidTestManager.register_default_scenarios = register_default_scenarios
    GstValidateAndroidTestManager.register_compositing_tests = register_compositing_tests
    GstValidateAndroidTestManager.register_default_encoding_formats = register_default_encoding_formats
except NameError:
    pass
