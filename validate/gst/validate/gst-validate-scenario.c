...
static GstValidateExecuteActionReturn
_execute_nested_action (GstValidateAction * action)
{
  ...
  if (subaction_struct) {
    ...
    if (!GST_CLOCK_TIME_IS_VALID (action->playback_time)) {
      ...
      GST_INFO_OBJECT (action->scenario,
          "Executing sub action %" GST_PTR_FORMAT, action->structure);
...
gst_validate_action_set_done (GstValidateAction * action)
{
  ...
  action->state = _execute_nested_action (action);
